...
export default function ReceivingOrderDetail() {
  ...
  const enrichedItems = useMemo(() => {
    ...
    return [{
      ...
      division: currentLog.inventoryItem?.divisions?.[0]?.divisionName || 'Unassigned Division',
      category: currentLog.inventoryItem?.categories?.[0]?.categoryName || 'Unassigned Category',
  ...
  // Safely passes both contentRef (v3) and content (v2) to ensure strict package compatibility
  ...
  const handlePrintTrigger = (mode) => {
    ...
    // 150ms timeout ensures React flushes the UI state update (adding/removing admin rows)
    // before react-to-print clones the element and opens the print dialog.
    setTimeout(() => {
      handlePrint();
    }, 150);
  };
  ...
  return (
    <>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {enrichedItems.map((item, i) => (
                      <tr key={i} className="hover:bg-white transition-colors">
                        ...
                        <td className="p-5 text-sm font-black text-slate-900 text-right">{item.totalLineWeight.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {/* ========================================================================= */}
      {/* We use position absolute/fixed off-screen instead of 'display:none' to guarantee 
          the ref is fully attached and styles are calculated before cloning. */}
          ...
          <div className="flex flex-row justify-between gap-x-12 text-[11pt]">
            ...
            <div className="w-1/2 flex flex-col gap-y-4">
              <div className="flex items-start">
                ...
                <div className="flex-1">
                  <span className="font-bold block text-[13pt] text-black leading-none mb-1">{enrichedItems[0].sku}</span>
                  <span className="font-bold block text-black mb-1">{enrichedItems[0].name}</span>
                  {currentLog.lot !== 'N/A' && <span className="font-bold block text-black">Lot {currentLog.lot}</span>}
                </div>
              </div>

              <div className="flex items-start">
                <span className="w-36 font-bold text-black shrink-0">Product Code:</span>
                <span className="flex-1 text-black">{enrichedItems[0].category}</span>
              </div>
              ...
              <div className="flex items-start mt-2">
                ...
                <span className="flex-1 text-black">{currentLog.numberOfCartons?.toLocaleString() || 0}</span>
              </div>
            ...
            <div className="w-1/2 flex flex-col gap-y-4">
              ...
              <div className="flex items-start">
                ...
                <span className="flex-1 text-black">{enrichedItems[0].division}</span>
              </div>
              ...
              <div className="flex items-start mt-[10px]">
                <span className="w-40 font-bold text-black shrink-0">Qty. Per Carton:</span>
                <span className="flex-1 text-black">{currentLog.unitsPerCarton?.toLocaleString() || 0}</span>
              </div>
