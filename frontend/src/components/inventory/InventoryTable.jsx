...
import { Edit, Trash2, ExternalLink, Package, Tag, User, DollarSign } from 'lucide-react';

export default function InventoryTable({ filteredInventory, apiInventory, onEditClick, onDeleteClick }) {
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-white/40 text-xs font-bold">
            {filteredInventory.length > 0 ? (
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl bg-white/60 border border-slate-200/60 flex items-center justify-center shrink-0 text-slate-400 shadow-sm overflow-hidden">
                          {rawItem?.productImage ? (
                            <img src={rawItem.productImage} alt="" className="w-full h-full object-cover"/>
                          ) : (
                            <Package size={16} />
                          )}
                        </div>
              ...
              <tr>
                <td colSpan={6} className="p-12 text-center font-bold text-slate-500 text-sm bg-white/10">
