...
import { ShieldCheck, Building2, Search, Filter, MapPin, UserPlus, Upload } from 'lucide-react';
import BulkUserUploadModal from './user-controls/BulkUserUploadModal';
  ...
  return (
    <>
      <div className="bg-white/60 backdrop-blur-2xl border border-white/60 p-4 rounded-3xl flex flex-col 2xl:flex-row 2xl:items-center gap-4 shadow-sm">
        ...
        {/* Action Buttons */}
        {canManageUsers && (
          <div className="flex flex-wrap items-center gap-3 ml-auto shrink-0 w-full sm:w-auto">
            <button 
              onClick={() => setShowBulkModal(true)}
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-white text-slate-700 border border-slate-200 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider hover:bg-slate-50 transition-all shadow-sm whitespace-nowrap active:scale-95"
            >
              <Upload size={14} /> Bulk Upload
            </button>
            <button 
              onClick={handleOpenCreateModal}
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-slate-900 text-white px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/10 whitespace-nowrap active:scale-95"
            >
